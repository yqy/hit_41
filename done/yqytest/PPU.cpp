...
void PPU6528::ScanLine(LPBYTE pBit, int LineNo)
{
	...
	{
                ScanSprite(pBit, LineNo, true);	//É¨ÃèºóÌ¨¾«Áé
	}
	if (LineNo < 240 && m_REG[1] & PPU_SHOWBG)
		ScanBG(pBit, LineNo);			//É¨Ãè±³¾°Ò³
	if (LineNo < 240 && m_REG[1] & PPU_SHOWSPR)
	{
                ScanSprite(pBit, LineNo, false);	//É¨ÃèÇ°Ì¨¾«Áé
		ScanHitPoint(LineNo);			//É¨ÃèHitµã
	}
}
...
{
	int sp_h = (PPU.m_REG[0] & PPU_SP16_BIT) ? 15 : 7;		//¾«Áé³ß´ç£¬7»ò15
...
{
	BYTE NTIndex = (m_CurByteIndex >> 10) & 0x3d;	
